...
- [Cleared Index](https://clearedindex.com) — Protocol-agnostic endpoint discovery + trust layer. Publishes `/.well-known/x402.json`, supports trust evaluations at `POST /api/cleared/trust/evaluate`, and returns signed attestations verifiable via `GET /api/cleared/jwks`.
