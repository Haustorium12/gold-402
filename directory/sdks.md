...
### Routing
- [use-agently](https://github.com/agentlyhq/use-agently) — Routing and settlement layer for AI agents. Pay x402 APIs from any agent framework. 69★
...
### Official
- [x402](https://pypi.org/project/x402/) — Official Python SDK. FastAPI middleware, `requests` session with auto-payments, payment requirement generation.

### Community
- [ag402](https://github.com/AetherCore-Dev/ag402) — Payment layer for AI agents using x402. Wrap any API or MCP server with a USDC paywall (`ag402 serve`), or let agents auto-pay (`ag402 run`). Solana USDC, ~0.5s settlement, non-custodial, 648+ tests.
...
- [openai-agents-nano-x402](https://github.com/PANDeveloper001/openai-agents-nano-x402) — Free feeless Nano (XNO) x402 payer for OpenAI Agents SDK. Pay per API call in Nano — drop-in `make_nano_x402_tool` wraps any function or agent. Instant settlement, zero fees, no bridge. ([pip](https://pypi.org/project/openai-agents-nano-x402/))
...
- [r402](https://github.com/qntx/r402) — Rust SDK for the x402 payment protocol by qntx. High-performance, type-safe implementation. 147★
- [ra2a](https://github.com/qntx/ra2a) — Rust SDK for the Agent2Agent (A2A) Protocol with x402 integration. 165★
- [x402-rs](https://github.com/x402-rs/x402-rs) — Production-grade Rust implementation. Axum middleware, reqwest client wrapper, self-hostable facilitator, multi-chain support.
- [x402-kit](https://github.com/bitrouter/x402-kit) — Fully modular, framework-agnostic Rust SDK. Composable buyer (signer) and seller (server) primitives. ([crates.io](https://crates.io/crates/x402-paywall))
...
- [x402-foundation/x402 (Go)](https://github.com/x402-foundation/x402/tree/main/go) — Official Go implementation. Core protocol types, multi-chain support.
- [x402-go](https://github.com/mark3labs/x402-go) — Community Go implementation by Mark III Labs.
- [mcp-go-x402](https://github.com/mark3labs/mcp-go-x402) — x402 payment transport for MCP-Go clients and servers. Drop-in replacement compatible with mcp-go transport interface. Automatic 402 handling, multi-chain (EVM + Solana), mock signers for testing. ([Go Package](https://pkg.go.dev/github.com/mark3labs/mcp-go-x402))

---

## Java

- [Mogami](https://mogami.tech) — Production-ready Java x402 stack. Includes SDK, server, console, and a bundled MCP server for Claude and OpenAI. Fills the Java gap in the official x402 ecosystem.

---

## .NET

- [x402-dotnet](https://github.com/michielpost/x402-dotnet) — x402 Payment Protocol implementation for .NET. Fills the .NET language gap in the ecosystem. 11★
...
- [Solana Foundation Pay](https://github.com/solana-foundation/pay) — Official Solana Foundation library for handling x402 and MPP payment challenges with user-authorized stablecoin signing. Updated May 2026. Stream payments, not transactions — 1,000 requests = only 2 on-chain transactions.
- [pay-kit](https://github.com/solana-foundation/pay-kit) — Agentic commerce building blocks (x402, MPP, AP2) for TypeScript, Rust, Go, Python. Official Solana Foundation toolkit covering all major agent payment protocols. 63★
- [tryx402](https://pypi.org/project/tryx402/) — Budget router and execution gateway over x402: hard session budget caps, idempotent paid calls, cost ledger, multi-currency fiat billing with margin. Python SDK + CLI + MCP server ([GitHub](https://github.com/crypto-yannso/tryx402)).
