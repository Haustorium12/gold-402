...
> The gold standard for x402 resources. **<!--COUNT:START-->569<!--COUNT:END--> curated entries** — paid endpoints probed for a live 402 before listing, libraries and repos checked for real activity, and the whole shelf re-knocked every night with the result dated. No filler.
...
The product. <!--COUNT:START-->569<!--COUNT:END--> entries across <!--SHELVES:START-->13<!--SHELVES:END--> shelves, in [`directory/`](directory/).
...
- **[webcap](https://webcap.shoutsid.fyi/.well-known/x402)** — Web capture API with seven pay-per-call routes: page screenshots, structured content extraction, SEO audits, site mapping, scroll-capture video, and AI visual analysis, $0.001-$0.01 USDC on Base via x402 with no API keys. Example: `POST /v1/x402/capture {"url":"https://example.com"}`.
- **[Crypto Sentiment API](https://crypto-sentiment-x402.onrender.com/sentiment/BTC)** — Real-time crypto sentiment for any ticker, aggregating 8 crypto news outlets and the Fear & Greed Index into a bullish/bearish/neutral score. $0.01 USDC on Base, no API key.
- **[PayPerByte](https://x402.payperbyte.io/.well-known/x402.json)** — Twelve pay-per-call USDC data feeds and oracles on Base mainnet: weather, earthquakes, CCTP v2 attestation-latency distributions, security advisories (CVE + CISA KEV), runtime end-of-life dates, malicious-package verdicts, perp positioning, and Know-Your-Agent sanctions/counterparty/merchant-screening and reasoning-verdict oracles; $0.003–$0.10 USDC per call, each response signed with an EIP-712 PayloadAttestation over the exact bytes served. Example: `POST /feeds/address-reputation {"domain":"example.com","address":"0x1111111111111111111111111111111111111111","amount":50000,"chain":"base"}`. ([GitHub](https://github.com/0rkz/byte-x402-gateway)) ([Manifest](https://x402.payperbyte.io/.well-known/x402))
- **[Clarity](https://desktop-o99r0sf.tail935fba.ts.net/.well-known/x402)** — Base mainnet x402 gateway for external USDC payers: OpenAI-compatible chat ($0.001) and short research/verdict reports ($2) via local Ollama. No API keys. Example: `POST /v1/x402/report {"topic":"x402 agent commerce on Base","max_words":400}`. ([OpenAPI](https://desktop-o99r0sf.tail935fba.ts.net/openapi.json)) ([Buy](https://desktop-o99r0sf.tail935fba.ts.net/static/buy.md)) ([Buyer walkthrough](https://desktop-o99r0sf.tail935fba.ts.net/static/buyer.md))
